...
import { clearReportHistory, readReportHistory, REPORT_HISTORY_UPDATED_EVENT, type ReportHistoryEntry } from "@/lib/reportHistory";
...
export function ReportHistoryPanel() {
  ...
  return <section className="report-history-panel paper-card" aria-labelledby="report-history-title">
    ...
    <div className="report-history-list">{entries.map((entry) => <article key={entry.id}><div><strong>{entry.companyName}</strong><span>Created {new Date(entry.generatedAt).toLocaleDateString()} · {formatEstimate(entry.totalPotentialValue)} potential value</span></div><div className="report-history-actions"><Link href={`/report/${entry.id}`}><ExternalLink size={14} /> View</Link><button type="button" disabled={download.isPending} onClick={() => reDownload(entry)}><Download size={14} /> PDF</button></div></article>)}</div>
